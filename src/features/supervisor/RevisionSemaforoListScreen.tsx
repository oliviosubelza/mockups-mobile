...
export interface SemaforoAuditItem {
  ...
  counts: {
    ...
    consolidator: { status: 'COMPLETED' | 'PENDING'; user?: string; time?: string };
    semaphoreAuditor: { status: 'COMPLETED' | 'PENDING'; user?: string; time?: string };
  };
}
...
const MOCK_SEMAFORO_ORDERS: SemaforoAuditItem[] = [
  {
    ...
    status: 'PENDING',
    counts: {
      driver: { status: 'COMPLETED', user: 'Cristhian Macchiavelli (Chofer)', time: '13:10' },
      consolidator: { status: 'COMPLETED', user: 'Carlos Mendoza (Supervisor)', time: '14:20' },
  ...
  },
  {
    ...
    status: 'COMPLETED',
    counts: {
      driver: { status: 'COMPLETED', user: 'Roberto Gómez (Chofer)', time: '10:45' },
      consolidator: { status: 'COMPLETED', user: 'Laura Vargas (Supervisor)', time: '11:15' },
      semaphoreAuditor: { status: 'COMPLETED', user: 'Juan Pérez (Auditor)', time: '12:00' },
    },
  },
  {
    id: 'sem-3',
    ...
    status: 'COMPLETED',
    counts: {
      driver: { status: 'COMPLETED', user: 'Cristhian Macchiavelli (Chofer)', time: '12:00' },
      consolidator: { status: 'COMPLETED', user: 'Carlos Mendoza (Supervisor)', time: '12:45' },
      semaphoreAuditor: { status: 'COMPLETED', user: 'Juan Pérez (Auditor)', time: '13:30' },
    },
  },
  {
    id: 'sem-4',
    ...
    status: 'PENDING',
    counts: {
      driver: { status: 'COMPLETED', user: 'Cristhian Macchiavelli (Chofer)', time: '09:00' },
...
export default function RevisionSemaforoListScreen() {
  ...
  const [activeTypeFilter, setActiveTypeFilter] = useState<'all' | 'cold'>('all');
  ...
  const totalColdCount = MOCK_SEMAFORO_ORDERS.filter((o) => o.isColdChain).length;
      ...
      >
        ...
        >
          ...
          <Text variant="caption" style={{ fontSize: 12, color: theme.colors.mutedForeground }}>
            Auditoría física realizada por el Supervisor/Auditor sin visualizar las cantidades esperadas.
          </Text>
        </View>
        ...
        >
          ...
          <TouchableOpacity
            ...
            style={{
              ...
              paddingVertical: 6,
              borderRadius: 20,
              backgroundColor: activeTypeFilter === 'all' ? theme.colors.primary : theme.colors.cardBackground,
              borderWidth: 1,
              borderColor: activeTypeFilter === 'all' ? theme.colors.primary : theme.colors.border,
            }}
          >
            <Text
              style={{
                fontSize: 12,
                fontWeight: '700',
                color: activeTypeFilter === 'all' ? '#ffffff' : theme.colors.foreground,
              }}
            >
          ...
          >
            <Snowflake
              size={13}
              color={activeTypeFilter === 'cold' ? '#ffffff' : theme.colors.primary}
            />
            <Text
              style={{
                fontSize: 12,
            ...
            return (
              ...
              >
                ...
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <Badge label={`${order.totalProducts} Productos`} tone="neutral" emphasis="soft" size="sm" />
                </View>
                  ...
                  <View style={{ gap: 4, paddingLeft: 4 }}>
                    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                      ...
                      <Text variant="caption" style={{ fontSize: 11, color: theme.colors.foreground }}>
                        <Text style={{ fontWeight: '700' }}>1º Chofer:</Text> {order.counts.driver.user} ({order.counts.driver.time})
                      </Text>
                    </View>
                    ...
                    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                      ...
                      <Text variant="caption" style={{ fontSize: 11, color: theme.colors.foreground }}>
                        <Text style={{ fontWeight: '700' }}>2º Supervisor Consolidador:</Text>{' '}
                        {order.counts.consolidator.user || 'Pendiente de consolidación'}
                      </Text>
                    </View>
                  </View>
                </View>
